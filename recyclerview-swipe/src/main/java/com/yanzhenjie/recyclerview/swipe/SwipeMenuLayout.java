...
public class SwipeMenuLayout extends FrameLayout implements SwipeSwitch {
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_UP:
                ...
                Log.d("andy", "滑动速度menu:" + velocity + "移动距离：" + dx + "最小Fill速度:" + mScaledMinimumFlingVelocity + "最大:" + mScaledMaximumFlingVelocity);
                if (velocity > mScaledMinimumFlingVelocity) {
                    if (mSwipeCurrentHorizontal != null) {
                        int duration = getSwipeDuration(ev, velocity);
                        //抬起时速度大于最大速度的一半 则关闭
                        if (velocity > mScaledMaximumFlingVelocity / 2) {
                            smoothToDeleteMenu(duration);
                            //return false;
                        } else if (mSwipeCurrentHorizontal instanceof SwipeRightHorizontal) {
                            if (velocityX < 0) {
    ...
    private void smoothToDeleteMenu(int duration) {
        if (mSwipeCurrentHorizontal != null) {
            ...
            /*RecoverAnimation rv = new RecoverAnimation(mHolder, ItemTouchHelper.ANIMATION_TYPE_SWIPE_SUCCESS, ItemTouchHelper.ACTION_STATE_IDLE
                    , getScrollX(), 0, 1000, 0);
            rv.setDuration(3000);
            rv.start();*/
        }
    ...
    private void changeWidth(int width) {
        View menu = mSwipeCurrentHorizontal.getMenuView();
    ...
    public void scrollTo(int x, int y) {
        if (mSwipeCurrentHorizontal == null || isDelete) {
            super.scrollTo(x, y);
        } else {
...
//            int menuViewHeight = contentViewHeight == 0 ? leftMenu.getMeasuredHeightAndState() : contentViewHeight;
    ...
    public void setNowHolder(RecyclerView.ViewHolder holder) {
        this.mHolder = holder;
    }
