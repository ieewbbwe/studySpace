...
class SwipeRightHorizontal extends SwipeHorizontal {
    ...
    public boolean isMenuOpen(int scrollX) {
        int i = -getMenuView().getWidth() / 3 * getDirection();
        return scrollX >= i && i != 0;
    }

    @Override
    public boolean isMenuOpenNotEqual(int scrollX) {
        return scrollX > -getMenuView().getWidth() / 3 * getDirection();
    }

    @Override
    public void autoOpenMenu(OverScroller scroller, int scrollX, int duration) {
        scroller.startScroll(Math.abs(scrollX), 0, getMenuView().getWidth() / 3 - Math.abs(scrollX), 0, duration);
    }
    ...
    @Override
    public void autoDeleteMenu(OverScroller scroller, int scrollX, int duration) {
    ...
    public boolean isClickOnContentView(int contentViewWidth, float x) {
        return x < (contentViewWidth - getMenuView().getWidth() / 3);
    }
}
