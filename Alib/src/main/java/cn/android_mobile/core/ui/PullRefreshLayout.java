...
/**
 * 实现接口 OnRefreshListener
 * @Override
	public void onRefresh() {
		//下拉刷新 调用，手动刷新调用
	}
 * 
 * iPulltoRefresh =(PullRefreshLayout)findViewById(R.id.pulltofresh);
   iPulltoRefresh.setOnRefreshListener(this);
   
	iPulltoRefresh.doRefresh(); 手动刷新
	iPulltoRefresh.onComplete(new Date().toLocaleString());
 * @author fyygw
 *
 */
...
//	private final int STATE_Hide= 4;
	...
	private void init(Context context) {
		mRefreshView = LayoutInflater.from(getContext()).inflate(R.layout.pull_to_refresh_header_old, null);
...
//				removeCallbacks(this);
	...
	private void scrollToClose() {
		...
		mFling.startUsingDistance(mTatolScroll, 750);
	}
	...
	public void doRefresh() {
		mState = STATE_REFRESHING;
		scrollToUpdate();
		//内容视图的复位操作差异性很大，为此将复位操作控制权交给外层由onRefresh自己负责
//		final View contentView = (View) getChildAt(1);
//		int viewScrollY =  contentView.getScrollY();
//		if(viewScrollY != 0)
